...
still works when bundled inside the Codex plugin and invoked from a bootcamp
...
def render_with_fpdf2(recap: Recap, output: Path) -> bool:
    ...
    class RecapPDF(FPDF):
        ...
        def footer(self) -> None:
            ...
            if self.page_no() == 1:
                self.cell(
                    0, 6, "Generated by the Senzing Bootcamp Codex plugin", align="C"
                )
            else:
...
def _cert_attribution(recap: Recap) -> List[str]:
    ...
    if version:
        lines.append(f"Senzing Bootcamp Codex plugin v{version.lstrip('v')}")
    return lines
