...
- **Plugin version:** the `version` field of the plugin manifest, resolved exactly as
  `onboarding-flow.md` step 0 specifies — `<plugin-root>/.codex-plugin/plugin.json`, else
  `<this-skill-dir>/../../.codex-plugin/plugin.json`, else "Unknown". ⛔ Never found by searching
...
- *Would this still happen with a perfect bootcamp plugin **and** a perfect Senzing MCP server?*
  If yes → **host** (the bootcamper's Codex interface owns it; neither component ships it).
...
Codex harness survives a perfect plugin *and* a perfect server, so a two-question test lands
...
| `host` | The bootcamper's **Codex interface** owns it — a harness prompt, dialog, toggle or session control that neither the bootcamp nor Senzing ships, and neither can fix | The Codex "Set up auto mode for your environment?" prompt appearing over a pending 👉 question during the onboarding preface (reported twice on 2026-08-15) |
...
- **Model / effort:** [model ID and reasoning-effort level; e.g. "configured Codex model / high", or "Unknown"]
...
reaches **Senzing**. A report about the Codex harness sent there arrives at a party that does
