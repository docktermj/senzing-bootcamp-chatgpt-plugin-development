...
- **Model/effort tuning.** Model and reasoning effort are host-level choices the bootcamper controls with the visible Codex controls (see `../../docs/model-selection.md`). At each module start you **proactively** surface this stage's
  ...
  Mapping, and Transformation through graduation — warrant a high-capability Codex model + high effort; the lighter
  conversational and collection stages a balanced Codex model.
...
- ⛔ **Every 👉 question traces to a step in a shipped skill file — never originate one (INV-247).**
  ...
  mode, background tasks, `/compact`, `/loop`. Those belong to the bootcamper's Codex task, not
  ...
  "Module start banners and transitions"), which is the only Codex-interface control the bootcamp
  ...
  - ⚠️ **Why this is a rule and not an assumption.** Observed twice on 2026-08-15: the Codex
...
- **A host control the bootcamper raises — in any form:** whether they **ask about** auto mode,
  ...
  any other setting belonging to their Codex task rather than to the bootcamp, **or tell you
...
- **Ask-once:** ask each question only once. Do not re-ask a question the bootcamper already
  ...
  - **A pending, *unanswered* question is different — re-present it verbatim.** After any
    ...
    detour, a host-rendered prompt from their Codex interface appearing over the bootcamp, or the
    ...
    ⚠️ **The host-rendered case is the one you may never see.** A prompt the bootcamper's Codex
