...
This is the Codex-plugin port of the Kiro `module-completion*` / `module-completion-artifacts` steering. It is deliberately lightweight and non-blocking. If any write fails, name what failed, do not claim the module is complete, and let the bootcamper decide how to proceed.
...
```bash
python3 "<plugin-root>/scripts/generate_recap_pdf.py" --check
# or, if plugin-root path is unset: python3 <this-skill-dir>/../../scripts/generate_recap_pdf.py --check
```
...
1. Run the bundled helper. Prefer the **live app's `localhost` URL** while the server is still up,
   ...
   Resolve `<helper>` as `<plugin-root>/scripts/capture_screenshots.py` (command/hook
