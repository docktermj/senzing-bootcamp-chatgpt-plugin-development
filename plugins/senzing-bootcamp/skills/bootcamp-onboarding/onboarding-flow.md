...
Read the plugin version from the plugin manifest (`.codex-plugin/plugin.json`, the `version`
field) and hold it to display with the WELCOME banner (step 3) and to record in the recap.
**Resolve the manifest as `<this-skill-dir>/../../.codex-plugin/plugin.json`.** This skill-relative path identifies the installed plugin deterministically. Never search the filesystem for another manifest. Use `Unknown` if that exact file cannot be read.
...
- **Failure** (timeout or error): display a blocking message and STOP. Do not proceed until the
  ...
  2. Confirm the "senzing" MCP server is configured and enabled in Codex
