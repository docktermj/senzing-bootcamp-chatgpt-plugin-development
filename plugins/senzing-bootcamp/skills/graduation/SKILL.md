...
`<plugin-root>/docs/examples/bootcamp_recap.example.pdf` (skill-relative
...
`<plugin-root>/docs/examples/bootcamp_recap.example.md` (skill-relative fallback:
...
- `**Plugin version:**` — from the plugin manifest (should already be present from the recap
  ...
  `<plugin-root>/.codex-plugin/plugin.json`, else
  `<this-skill-dir>/../../.codex-plugin/plugin.json`, else "Unknown" — and ⛔ never by searching
...
```bash
python3 "<plugin-root>/scripts/normalize_docs_markdown.py"
# or, if plugin-root path is unset: python3 <this-skill-dir>/../../scripts/normalize_docs_markdown.py
```
...
python3 "<plugin-root>/scripts/generate_recap_pdf.py"
# Or, when you installed fpdf2 into the project-local venv above:
data/temp/recap-venv/bin/python "<plugin-root>/scripts/generate_recap_pdf.py"
```

If `<plugin-root>` is not set in the current context, resolve the script
...
```bash
python3 "<plugin-root>/scripts/normalize_docs_markdown.py" --docs-dir production
# or, if plugin-root path is unset: python3 <this-skill-dir>/../../scripts/normalize_docs_markdown.py --docs-dir production
```
...
```bash
python3 "<plugin-root>/scripts/generate_document_pdf.py" \
...
# or, if plugin-root path is unset: python3 <this-skill-dir>/../../scripts/generate_document_pdf.py …
```
