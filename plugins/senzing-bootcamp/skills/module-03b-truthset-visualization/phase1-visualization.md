...
7. **Model/effort (INV-063/INV-137/INV-138):** surface this module's recommended model/effort per
   ...
   question; that became false when this module was re-rated to a high-capability Codex model / high effort while System
   verification stayed on a balanced Codex model, so a bootcamper who took the previous module's recommendation
...
- **Render offline (INV-091):** inline the vendored D3 at
  `<plugin-root>/scripts/vendor/d3.v7.min.js` (skill-relative fallback:
...
- **Use the Senzing brand (INV-081):** take the palette and typography from the shipped brand
  tokens (`<plugin-root>/scripts/brand_tokens.py`, skill-relative fallback
...
- **Python** → the reference implementation *is* the server. Resolve `scripts/senzing_viz_server.py`
  (`<plugin-root>/scripts/senzing_viz_server.py` in a command/hook context, else
...
`<plugin-root>/scripts/capture_screenshots.py` (INV-185; skill-relative fallback
...
```bash
python3 "<plugin-root>/scripts/capture_screenshots.py" \
