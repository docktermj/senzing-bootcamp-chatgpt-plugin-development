...
`<plugin-root>/scripts/brand_tokens.py` (INV-081; skill-relative fallback
`../../scripts/brand_tokens.py`, INV-252); **renders offline**, so no CDN or web font
— inline the vendored `<plugin-root>/scripts/vendor/d3.v7.min.js` (skill-relative fallback
