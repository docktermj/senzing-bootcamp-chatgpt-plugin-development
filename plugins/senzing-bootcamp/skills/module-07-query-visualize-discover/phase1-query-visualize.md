...
- Render offline with the vendored D3 asset inlined, no CDN (INV-091), and take palette/typography
  from `<plugin-root>/scripts/brand_tokens.py` (INV-081; skill-relative fallback
...
```bash
python3 "<plugin-root>/scripts/generate_discoveries_pdf.py"
# or, if plugin-root path is unset: python3 <this-skill-dir>/../../scripts/generate_discoveries_pdf.py
```
